...
@[simp]
theorem isIncreasing₂_singleton {α : Type*} [LT α] (a : Fin 3 → α) : IsIncreasing₂ [a] := by simp [IsIncreasing₂]
...
@[simp]
theorem tripleProduct_const {α : Type*} (a : α) : tripleProduct (fun _ => a) (fun _ => a) = toLex (fun _ => (a, a)) := by
...
/- An attempt to allow infinite length increasing sequences. Not really required for this
paper, and a bit tough to deal with the structures defined here. -/

/--
Since the $2$-less relation is not transitive, we make a further definition to
specify transivity.

We define increasing triples that may be infinite.
The core data underlying this structure is the sequence of values `seq` and the
property that this sequence is increasing. In order to allow for both finite and
infinite sequences, we also bundle a `length : ℕ∞` and specifying property
for this length. This implementation is motivated by later constraints, in
which we consider only sequences which can take values in some finite set.
If we do not include the `i < length` condition in `increasing` this will
lead to no such `IncreasingTriple` existing because we cannot have
an infinite sequence of increasing triples taking finitely-many values.
Conversely, if we remove that constraing, we may well have
infinite sequences of increasing triples.
-/
structure IncreasingTriple (α : Type*) [LT α] where
  seq : ℕ → Fin 3 → α
  length : ℕ∞
  length_eq : sSup { (N : ℕ∞) | ∀ (i : ℕ) (_ : i < N), seq i <₂ seq i.succ } = length
  increasing : ∀ (i : ℕ) (_ : i < length), seq i <₂ seq i.succ

theorem IncreasingTriple.length_congr {α : Type*} [LT α] {a b : IncreasingTriple α}
    (h : a.seq = b.seq) : a.length = b.length :=
  a.length_eq ▸ h ▸ b.length_eq

instance (α : Type*) [LT α] :
    FunLike (IncreasingTriple α) ℕ (Fin 3 → α) where
  coe := IncreasingTriple.seq
  coe_injective' a b h := by
    cases a
    congr
    exact IncreasingTriple.length_congr h

theorem card_le_of_mem_Icc {α : Type*} [DecidableEq α] [PartialOrder α] [LocallyFiniteOrder α]
    {a : IncreasingTriple α} {l u : α} (h : ∀ i, a i ∈ Finset.Icc (fun _ => l) (fun _ => u)) :
    a.length ≤ (Finset.Icc (fun (_ : Fin 3) => l) (fun _ => u)).card := by
  -- pigeonhole principle will give a contradiction with a.increasing otherwise
  sorry
...
-/
noncomputable def maximalLength' (n : ℕ) : ℕ :=
  sSup { IncreasingTriple.length a | (a : IncreasingTriple ℕ) (_ : ∀ i, a i ∈ Finset.Icc (fun _ => 1) (fun _ => n)) }
